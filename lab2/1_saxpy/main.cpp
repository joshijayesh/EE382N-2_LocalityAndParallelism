...
void saxpyCpu(long N, float alpha, float* x, float* y, float* result) {
    ...
    double timeCPU = endTime - startTime;
    timeCPUAvg += timeCPU;
}
...
int main(int argc, char** argv)
{
    ...
    if (resultarray != NULL) {
        ...
        printf("CPU SAXPY: %8.3f ms [%8.3f Ops/s]\n", 1000.f * timeCPUAvg, toBW(totalBytes/3, timeCPUAvg));
